...
function NetworkGuide() {
  return <><h3>快速进入房间</h3><ul><li>房主创建 4–10 位字母或数字房间号，朋友输入同一房间号即可加入。</li><li>所有玩家准备后，由房主开始游戏。</li><li>大厅的练功房是仅创建者可进入的私人房间。可增删假人、设置初始角色与资源，并在每回合依次替所有角色选择行动。</li><li>游戏中意外断线会保留席位 30 秒并自动重连；等待阶段断线直接离开。</li><li>结算后每名在房玩家确认结果，房间随后返回准备阶段。</li><li>右上角显示在线状态、往返延迟和战斗日志。地址后添加 <code>?perf=1</code> 可查看 FPS、慢帧、长任务和内存指标。</li></ul><p className="warning">用户名登录没有密码保护。任何知道用户名的人都可以进入同一账号。</p></>;
}
