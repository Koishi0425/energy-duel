...
import { experienceRequiredForLevel, MAX_PROFILE_LEVEL, PROFILE_NAMEPLATES, PROFILE_TITLES, type PlayerProfile, type SessionResponse } from '@energy-duel/shared';
...
export default function ProfilePage({ session, onBack, onProfileChange }: Props) {
  ...
  const currentLevelStart = experienceRequiredForLevel(profile.level);
  const levelProgress = profile.level >= MAX_PROFILE_LEVEL ? 100 : Math.max(0, Math.min(100, (profile.experience - currentLevelStart) / Math.max(1, profile.experienceForNextLevel - currentLevelStart) * 100));
